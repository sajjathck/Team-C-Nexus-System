...
const GetExamByExamIdAndStudentId = () => {
    ...
    const handleSearch = async () => {
        try {
            ...
            const studentResponse = await axios.get(`http://localhost:5225/api/Student/GetStudentById/${selectedStudentId}`);
            const studentDetails = studentResponse.data;

            // Fetch student marks
            const marksResponse = await axios.get(`http://localhost:5225/api/Examination/GetAllResultByStudIdAndExamId/${selectedStudentId}/${selectedExamId}`);
    ...
    return (
        <Container>
            <ToastContainer />
            <h2 className="mt-5 mb-3">GetExamByExamIdAndStudentId</h2>
            <Row className="mb-3">
                <Col>
...
export default GetExamByExamIdAndStudentId;
